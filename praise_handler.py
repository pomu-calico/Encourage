...
def register_praise_handlers(app, REFERRAL_CHANNEL_ID, PRAISE_CHANNEL_ID):
    ...
    def handle_reaction(event, client):
        ...
        target_reactions = ["獲得", "獲得2", "獲得最高", "獲得キター"]
        ...
        # 条件2：リアクションが「獲得」
        if reaction not in target_reactions:
            return
        
        # 条件3：獲得リアクションが1個目
        reaction_info = client.reactions_get(
            channel=channel_id,
            timestamp=ts
        )
        item_reactions = reaction_info["message"].get("reactions", [])
        total_count = sum(r["count"] for r in item_reactions if r["name"] in target_reactions)
